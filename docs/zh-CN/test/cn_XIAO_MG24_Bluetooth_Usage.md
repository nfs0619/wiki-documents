...
Seeed Studio XIAO MG24 是一款强大的开发板，支持蓝牙 LE 5.3 和蓝牙 mesh，使其成为需要无线连接的各种 IoT 应用的理想选择。凭借其卓越的射频性能，XIAO MG24 在各种距离上提供可靠、高速的无线通信，使其成为短距离和长距离应用的多功能解决方案。在本教程中，我们将探索 XIAO MG24 蓝牙功能的基本特性，包括如何扫描附近的蓝牙设备、建立蓝牙连接以及通过该连接传输和接收数据。
...
低功耗蓝牙，简称 BLE，是蓝牙的一种节能变体。BLE 的主要应用是小数据量的短距离传输（低带宽）。与始终开启的蓝牙不同，BLE 除了在启动连接时外，始终保持在睡眠模式。
...
对于[SIG（蓝牙特殊兴趣小组）](https://www.bluetooth.com/specifications/gatt/services)中指定的所有类型、服务和配置文件，都有缩短的 UUID。但如果您的应用程序需要自己的 UUID，您可以使用这个[UUID 生成器网站](https://www.uuidgenerator.net/)生成它。
...
代码首先定义了一个事件处理函数`sl_bt_on_event`，它处理 BLE 堆栈生成的各种低功耗蓝牙（BLE）事件。该函数使用 switch 语句来区分事件类型，例如 BLE 设备启动时和从附近设备接收广告报告时。在接收到启动事件时，它初始化串行通信，配置天线控制的 GPIO 引脚，并开始使用指定参数扫描 BLE 设备。
...
#ifndef BLE_STACK_SILABS
#error "This example is only compatible with the Silicon Labs BLE stack. Please select 'BLE (Silabs)' in 'Tools > Protocol stack'."
#endif 与此同时，您可以在主要的移动应用商店中搜索并下载 **nRF Connect** 应用，该应用允许您的手机搜索并连接到蓝牙设备。
...
在上述程序中，你可以看到`sl_bt_gattdb_add_service()`用于创建服务器。参数是一个特定的 UUID：**0x1809**。在 GATT 规则中，**0x1809**表示温度计类型数据，同样特征值的 UUID：**0x2A1C**也有特殊含义。在 GATT 中，它表示温度测量。这符合我们温度值的情况，所以这里我定义为这样。你可以阅读[这里](https://files.seeedstudio.com/wiki/SeeedStudio-XIAO-ESP32S3/res/GATT.pdf)了解 GATT 为我们准备的一些特定 UUID 的含义。
