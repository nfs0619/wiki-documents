...
# Seeed Studio XIAO MG24 的蓝牙使用方法
...
Seeed Studio XIAO MG24 是一款强大的开发板，支持 Bluetooth LE 5.3 和 Bluetooth mesh，使其成为需要无线连接的各种 IoT 应用的理想选择。凭借其卓越的 RF 性能，XIAO MG24 可在各种距离上提供可靠的高速无线通信，使其成为短距离和长距离应用的多功能解决方案。在本教程中，我们将探索 XIAO MG24 蓝牙功能的基本特性，包括如何扫描附近的蓝牙设备、建立蓝牙连接以及通过该连接传输和接收数据。
...
如果您想使用另一个 XIAO MG24 作为客户端来接收来自服务器的消息，那么您可以对客户端 XIAO 使用以下程序。
...
我们需要准备两个 XIAO，一个作为服务器，一个作为客户端。这是作为服务器的示例程序。作为服务器的 XIAO 有以下主要任务。
...
void setup()
  ...
  // 开启天线功能 digitalWrite(RF_SW_PW_PIN, HIGH);

  delay(100);

  // HIGH -> 使用外部天线 / LOW -> 使用内置天线 digitalWrite(RF_SW_PIN, LOW);
}
...
// IEEE 11073 浮点结构 uint8_t mantissa_l;
  uint8_t mantissa_m;
  int8_t mantissa_h;
  int8_t exponent;
} IEEE_11073_float;
...
void sl_bt_on_event(sl_bt_msg_t *evt)
{
  ...
  switch (SL_BT_MSG_ID(evt->header)) {
    ...
    // 当我们扫描到另一个 BLE 设备的广播时接收此事件 Serial.println("收到 BLE 扫描报告");
    ...
    // 当 BLE 连接已打开时接收此事件 Serial.println("连接已打开");
      digitalWrite(LED_BUILTIN, LED_BUILTIN_ACTIVE);
    ...
    // 当 BLE 连接已关闭时接收此事件 Serial.println("连接已关闭");
      digitalWrite(LED_BUILTIN, LED_BUILTIN_INACTIVE);
    ...
    // 默认事件处理程序 Serial.print("BLE 事件：0x");
      Serial.println(SL_BT_MSG_ID(evt->header), HEX);
      break;
  }
}
...
static bool find_complete_local_name_in_advertisement(sl_bt_evt_scanner_legacy_advertisement_report_t *response)
{
  ...
  // 遍历响应数据 uint8_t advertisement_length = response->data.data[i];
    uint8_t advertisement_type = response->data.data[i + 1];

    // 类型 0x09 = 完整本地名称，0x08 缩短名称 if (advertisement_type == 0x09) {
...
static float translate_IEEE_11073_temperature_to_float(IEEE_11073_float const *IEEE_11073_value)
{
  ...
  // 错误参数：传递了 NULL 指针 return NAN;
  }

  // 缓存字段 mantissa_m = IEEE_11073_value->mantissa_m;
  mantissa_h = IEEE_11073_value->mantissa_h;
  exponent =  IEEE_11073_value->exponent;

  // 传递了 IEEE-11073 标准 NaN 值 return NAN;
  }

  // 将 24 位有符号值转换为 32 位有符号值 mantissa <<= 8;
  mantissa |= mantissa_m;
  mantissa <<= 8;
  mantissa |= mantissa_l;
  mantissa <<= 8;
...
// 温度测量特征 uint8_t temp_initial_value[5] = { 0, 0, 0, 0, 0 };
sc = sl_bt_gattdb_add_uuid16_characteristic(gattdb_session_id,
                                            service_handle,
                                            SL_BT_GATTDB_CHARACTERISTIC_INDICATE,
                                            0x00,
...
当然，你也可以不遵循 GATT 标准设置 UUID，你只需要确保这两个值是唯一的，不会影响你的客户端通过识别这些 UUID 找到值的能力。你可以访问[uuidgenerator.net](https://www.uuidgenerator.net/)为你的服务和特征创建随机 UUID。
