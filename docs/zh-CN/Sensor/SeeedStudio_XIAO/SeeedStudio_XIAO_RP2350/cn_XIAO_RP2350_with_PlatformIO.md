...
PlatformIO 是一个集成了多种开发板的开发平台，并且具有良好的扩展性。如果平台没有您需要的开发板类型，您可以手动添加开发板类型。您在 Arduino 上编写的代码可以直接使用，只需添加相应的库即可。

在本 Wiki 中，我们将介绍如何在 PlatformIO 中安装并运行示例代码。

## 使用 PlatformIO 开发 XIAO RP2350
...
由于平台已经包含了我们的 XIAO ESP32S3 和 XIAO ESP32C3 开发板选项，我们可以选择其中一个来创建文件。当然，其他文件也可以使用，这并不影响。项目名称可以随意选择。
...
:::tip
在此之前，我已经安装了 XIAO RP2350 的安装包，因此您可以看到操作二中的图片有 XIAO RP2350 的选项，但您在执行操作时可能没有该选项。
:::
...
最后，如果您看到与上图相同的结果，说明您已经成功添加了 XIAO RP2350 开发板。当您再次创建项目时，您将看到 XIAO RP2350 的选项可用。
