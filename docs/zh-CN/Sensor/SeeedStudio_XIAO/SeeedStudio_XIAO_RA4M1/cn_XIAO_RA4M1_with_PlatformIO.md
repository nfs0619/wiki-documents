---
description: Seeed Studio XIAO RA4M1 使用 PlatformIO
title: XIAO RA4M1 使用 PlatformIO
keywords:
- xiao
...
PlatformIO 是一个集成了多种开发板类型的开发平台，并且具有良好的扩展性。如果平台中没有您需要的开发板类型，您可以手动添加所需的开发板类型。您在 Arduino 上编写的代码可以直接使用，只需添加相应的库即可。

在本教程中，我们将介绍如何在 PlatformIO 中安装并运行示例代码。

## 使用 PlatformIO 配置 XIAO RA4M1

### 步骤 1. 在 [PlatformIO](https://platformio.org/platformio-ide) 官方网站安装软件

如果您尚未安装 PlatformIO 软件，可以点击上方链接进行安装。

### 步骤 2. 在 PlatformIO 中创建任意项目

由于平台已经包含了我们的 XIAO ESP32S3 和 XIAO ESP32C3 开发板选项，我们可以选择其中一个来创建文件。当然，其他文件也可以使用，这并没有关系。项目名称也可以随意选择。
...
:::tip
在此之前，我已经安装了 XIAO RA4M1 的安装包，因此您可以看到操作二中的图片有 XIAO RA4M1 的选项，但在您执行操作时可能没有该选项。
:::
...
最后，如果您看到与上图相同的结果，说明您已经成功添加了 XIAO RA4M1 开发板。当您再次创建项目时，您会看到 XIAO RA4M1 的选项可用。
