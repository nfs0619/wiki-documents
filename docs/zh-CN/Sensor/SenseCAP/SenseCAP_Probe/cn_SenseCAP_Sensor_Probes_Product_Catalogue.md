---
description: SenseCAP_Sensor_Probes_产品目录
title: SenseCAP 传感器探头产品目录
keywords:
...
# 工业级 MODBUS RS485 NH3 传感器 - 带防水航空连接器
...
作为 Seeed IIoT 产品系列的首批推出产品之一，SenseCAP 专注于无线环境传感应用：例如智能农业、精准农业和智慧城市。它包括硬件产品（传感器、数据记录器和网关等）、软件服务（SenseCAP Mate App、SenseCAP Web Portal、开放仪表盘）以及用于设备和数据管理的 API。

## 关于传感器探头

我们的 SenseCAP 工业传感器探头系列专为承受工业应用中常见的恶劣环境（如振动以及化学品和气体的暴露）而设计。我们的工业传感器系列具有从 IP66 到 IP68 的高等级防护等级，以确保产品在各自的恶劣环境中可靠运行。  
SenseCAP 传感器探头还具有工艺连接功能，以确保在被测介质中的安全安装，并配备传感器数据发射器，将信号转换为适合工业仪表系统的格式。  
部分 MODBUS RS485 工业传感器有两个版本。主要区别在于电缆连接器。请选择最适合您需求的版本：  
（1）带有接线端的版本建议与 [SenseCAP LoRaWAN® 数据记录器](https://www.seeedstudio.com/SenseCAP-S2100-LoRaWAN-Data-Logger-p-5361.html?queryID=82d7de6ca64b8316686bdfe2c05664d2&objectID=5361&indexName=bazaar_retailer_products) 一起使用，以连接到 LoRaWAN® 网络，享受低功耗和长距离通信的优势。  
（2）带有航空连接器的版本已预配置 [SenseCAP Sensor Hub 4G 数据记录器](https://www.seeedstudio.com/SenseCAP-Sensor-Hub-4G-Data-Logger-with-builtin-battery-p-4852.html?queryID=b8ef1fa7e141765c30033fc3d8eddf10&objectID=4852&indexName=bazaar_retailer_products)，提供即插即用体验，通过 3G/4G LTE 收集各种传感器数据。
...
[1] 从传感器通电到读取数据的时间。请注意传感器通电时的参数。

[2] 如果在清洁空气环境中启动，所需时间会更短。

[3] 测量数据更新间隔。在通电预热时间之后，如果电源持续供电，传感器会以此间隔周期性更新读数。

[4] T90＜30秒

## 工业级 MODBUS RS485 H2S 传感器 - 带防水航空连接器
...
## SenseCAP CO2、温度和湿度传感器，带 RS485 & SDI-12 接口，带防水航空连接器
...
[1] 传感器通电后准备读取通过 Modbus 指令测量值所需的时间，应注意传感器每次通电和断电时的参数。

[2] 预热时间 = （寄存器 0x0021 的值 + 3）；在预热时间内读取寄存器 0x0000 会得到 0，预热时间后每秒更新一次读数；传感器的 T90 时间为 300 秒，T60 时间为 120 秒。

[3] 测量数据更新间隔，在通电预热时间之后，如果持续供电，传感器会以此间隔周期性更新读数。

[4] Modbus 主机查询频率。

[5] 传感器在接收到读取指令后响应所需的时间，当响应延迟寄存器 0x0020 设置为 0 时。

## 工业级 MODBUS RS485 空气温度和湿度传感器
...
带接线 [101990668](https://www.seeedstudio.com/RS485-Soil-Moisture-Temperature-Sensor-S-Soil-MT-02-p-4634.html?queryID=d56c42134f064eaf1203b9292aaba7c8&objectID=4634&indexName=bazaar_retailer_products) 
...
## 工业级土壤湿度 & 温度 & 电导率传感器 MODBUS-RTU RS485 (S-Soil MTEC-02A)

带接线 [101990667](https://www.seeedstudio.com/RS485-Soil-Moisture-Temperature-EC-Sensor-S-Soil-MTEC-02-p-4633.html)  

带航空接头 [314990621](https://www.seeedstudio.com/RS485-Soil-Moisture-Temperature-EC-Sensor-S-Soil-MTEC-02B-p-4860.html?queryID=ec8fbdf14c91f77c70bd5e0177bb9620&objectID=4860&indexName=bazaar_retailer_products)  
...
## 工业级光强传感器，MODBUS-RTU RS485 & 0-2V (S-Light-01)

带接线 [314990739](https://www.seeedstudio.com/light-intensity-sensor-p-a-4863.html?queryID=f94dd76cedb45ec9d57e70d6e82af574&objectID=4863&indexName=bazaar_retailer_products)

带航空接头 [314990740](https://www.seeedstudio.com/light-intensity-sensor-p-b-4863.html)
...
## 工业级光合有效辐射传感器 (S-PAR-02A), MODBUS-RTU RS485

带接线 [314990733](https://www.seeedstudio.com/RS485-S-PAR-02A-p-4829.html)

带航空接头 [314990735](https://www.seeedstudio.com/RS485-S-PAR-02B-p-4830.html)
...
## 工业级光合有效辐射传感器 (PAR-2.5V)，支持模拟 0~2.5V 输出
...
## 工业级总太阳辐射传感器 (S-ZFS-02A), MODBUS-RTU RS485
...
## 工业紫外线辐射传感器 (S-ZW-01A)，MODBUS-RTU RS485
...
## 工业叶面湿度和温度传感器 (A)，MODBUS-RTU RS485

带接线 [314990737](https://www.seeedstudio.com/leaf-wetness-sensor-p-a-4861.html)

带航空接头 [314990738](https://www.seeedstudio.com/leaf-wetness-sensor-p-b-4861.html)
...
## 工业 pH 传感器 MODBUS-RTU RS485 & 0~2V 模拟电压 (S-pH-01A) V2.0

带接线 [101990666](https://www.seeedstudio.com/RS485-pH-Sensor-S-pH-01A-p-4632.html)

带航空接头 [314990622](https://www.seeedstudio.com/RS485-pH-Sensor-S-pH-01B-p-4632.html)
...
## 工业级 EC & TDS 传感器 MODBUS-RTU RS485 & 0-2V 模拟电压

带连接线 [314990634](https://www.seeedstudio.com/EC-RS485-p-4848.html)

带航空连接器 [314990742](https://www.seeedstudio.com/EC-&-TDS-Sensor-RS485-b-p-4865.html)
