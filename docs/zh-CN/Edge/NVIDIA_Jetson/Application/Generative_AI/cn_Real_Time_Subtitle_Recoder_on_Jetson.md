...
如今，我们每天都有许多会议，但有些会议内容不适合公开。将会议内容发送到云端进行记录并返回字幕可能会对会议隐私构成重大威胁。而且最重要的是，如果网络延迟，您可能会丢失会议内容。

<div align="center"><img width={800} src="https://files.seeedstudio.com/wiki/reComputer-Jetson/A608/recoder.gif" /></div>

在这里，我们向您介绍 [基于 Jetson 的语音字幕生成](https://github.com/yuyoujiang/Real-time-Subtitle-Recorder-on-Jetson)，它可以提供实时语音转字幕服务，同时避免因网络传输导致的信息泄露。会议内容可以通过 AI 模型以字幕形式转录并显示在屏幕上，从而保护会议隐私并减少会议记录的工作量。
...
<div class="get_one_now_container" style={{textAlign: 'center'}}>
    <a class="get_one_now_item" href="https://www.seeedstudio.com/reComputer-Industrial-J3011-p-5682.html?queryID=c1e6f0b0bd38a98233ce64bce8083a22&objectID=5682&indexName=bazaar_retailer_products"><strong><span><font color={'FFFFFF'} size={"4"}> 立即获取 🖱️</font></span></strong></a>
</div>
...
<div class="get_one_now_container" style={{textAlign: 'center'}}>
    <a class="get_one_now_item" href="https://www.seeedstudio.com/ReSpeaker-Mic-Array-v2-0.html?queryID=2baffb980bdb6d5e65b2b3f511657cb2&objectID=139&indexName=bazaar_retailer_products)"><strong><span><font color={'FFFFFF'} size={"4"}> 立即获取 🖱️</font></span></strong></a>
</div>
...
请参考 [此文档](https://wiki.seeedstudio.com/Local_Voice_Chatbot/#install-riva-server) 安装 Riva ASR 服务器。

在终端中（Ctrl+Alt+T），输入 ```sudo docker ps```，如果看到如下内容，说明您已完成第一步。
...
```
在终端中（Ctrl+Alt+T），输入 ```pip --version```，如果看到如下内容，说明您已完成 pip 升级。
...
在终端中（Ctrl+Alt+T），输入 ```python3 -c 'import setuptools; print(setuptools.__version__)```，如果看到如下内容，说明您已升级 setuptools。
...
在终端中（Ctrl+Alt+T），输入 ```wheel version```，如果看到如下内容，说明您已完成 wheel 升级。
...
在本项目中，我们使用 [Riva ASR Server](https://catalog.ngc.nvidia.com/orgs/nvidia/teams/riva/resources/riva_quickstart) 从麦克风输入中实时捕获数据，并将其显示在网页上。未来，我们将添加更多应用，例如将一种语言翻译成另一种语言，以及提高响应速度。