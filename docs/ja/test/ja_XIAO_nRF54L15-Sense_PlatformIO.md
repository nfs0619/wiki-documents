...
PlatformIO は、組み込みシステム向けに設計された強力で高度に拡張可能な開発エコシステムです。膨大な数の開発ボードとマイクロコントローラーのサポートをシームレスに統合し、比類のない柔軟性を提供します。PlatformIO を際立たせているのは、その優れたスケーラビリティです：特定のボードがネイティブにサポートされていない場合でも、そのアーキテクチャにより簡単にカスタムボード定義を作成できます。
...
以下の合理化された手順に従って、開発環境を構成し、XIAO nRF54L15 Sense で最初のアプリケーションをデプロイしてください。
...
### XIAO nRF54L15 Zephyr サポート用の platformio.ini の設定

プロジェクトが作成されたら、プロジェクトディレクトリのルートにある platformio.ini ファイルを見つけます（左側の VS Code エクスプローラーで確認できます）。このファイルは PlatformIO プロジェクト設定の中核です。
...
:::tip
重要なステップ：コードを貼り付けた後、platformio.ini ファイルを保存することを忘れないでください（Ctrl+S または Cmd+S）。PlatformIO は変更を自動的に検出し、platform-seeedboards GitHub リポジトリから必要な Zephyr フレームワークとボード固有のツールのダウンロードを開始します。この処理には少し時間がかかる場合があります。
:::


### 最初の Blink サンプルのコンパイルとアップロード

それでは、古典的な「Blink」サンプルでセットアップをテストしましょう。このコードは XIAO nRF54L15 の内蔵 LED を点滅させます。
...
- ビルド：VS Code 下部の PlatformIO ツールバーの「ビルド」アイコン（チェックマーク）をクリックするか、PlatformIO サイドバーを使用します：PROJECT TASKS -> your_project_name -> General -> Build。

- アップロード：ビルドが成功した後、PlatformIO ツールバーの「アップロード」アイコン（右矢印）をクリックするか、PlatformIO サイドバーを使用します：PROJECT TASKS -> your_project_name -> General -> Upload。
